...
export class CategoryService {
  ...
  async getAllCategories(): Promise<Category[]> {
    ...
    if (!categories.length) throw new AppError('No categories found', NOT_FOUND, NOT_FOUND_REASON);
    return categories;
  }

  async getCategoryById(id: string): Promise<Category> {
    const category = await this.categoryRepo.findOne({ where: { id } });
    if (!category) throw new AppError("Category not found", NOT_FOUND, NOT_FOUND_REASON);
    return category;
  }
